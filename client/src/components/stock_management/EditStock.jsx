...
import { ArrowLeft, Plus, Trash2, Upload, Save, Package } from "lucide-react";
...
export default function EditStock({ product, onBack, onStockUpdated }) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (product) {
          ...
          setFormData({
            referenceNo: `STR-EDT-${Date.now().toString().slice(-6)}`,
            date: new Date().toISOString().split("T")[0],
            status:
              stock === 0
                ? "Out of Stock"
            ...
            warehouse: "",
            storageLocation:
              product.warehouseLocation || product.binLocation || "",
            user: "",
          });
          ...
          setItems([
            {
              ...
              totalCost: stock * (product.purchasePrice || product.cost || 0),
              expiryDate: "",
            },
          ]);
        }
  ...
  const handleProductSelect = (index, selectedProduct) => {
    setItems((prev) => {
      ...
      updated[index] = {
        ...
        skuCode: selectedProduct.sku || selectedProduct.barcode || "",
        unit: selectedProduct.unit || "Nos",
        unitCost: selectedProduct.purchasePrice || selectedProduct.cost || 0,
        totalCost:
          updated[index].quantity *
          (selectedProduct.purchasePrice || selectedProduct.cost || 0),
      };
      return updated;
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (product && items[0]?.product) {
        const newStock = Number(items[0].quantity);
        await fetch(`${API_URL}/products/${product._id}`, {
          ...
          body: JSON.stringify({
            ...
            warehouseLocation: formData.warehouse || product.warehouseLocation,
            binLocation: formData.storageLocation || product.binLocation,
          }),
        });
      }
      ...
      if (data.success) {
        if (onStockUpdated)
          onStockUpdated({ ...product, currentStock: totalQuantity });
        onBack();
      } else {
