...
import { ArrowLeft, Save, Package, Calendar, MapPin, ClipboardCheck } from 'lucide-react';
...
export default function PhysicalAuditForm() {
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => {
      ...
      if ((name === 'systemQuantity' || name === 'physicalQuantity') && updated.systemQuantity && updated.physicalQuantity) {
        const diff = parseInt(updated.physicalQuantity) - parseInt(updated.systemQuantity);
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      setLoading(false);
      alert('Physical Audit entry created successfully!');
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[18px] font-extrabold text-slate-900 tracking-tight">Add Physical Audit</h1>
            <p className="text-[11px] text-slate-500 font-semibold">Record physical stock audit entry</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleSubmit} disabled={loading}
            ...
            <Save size={14} />{loading ? 'Saving...' : 'Save Audit'}
          </motion.button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Section title="Audit Details" icon={ClipboardCheck}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Field label="Audit Type" required>
              <select name="auditType" value={form.auditType} onChange={handleChange} className={inputCls} required>
            ...
            <Field label="System Quantity (Book)" required>
              <input type="number" name="systemQuantity" value={form.systemQuantity} onChange={handleChange} placeholder="0" min="0" className={inputCls} required />
            </Field>
            <Field label="Physical Quantity (Actual)" required>
              <input type="number" name="physicalQuantity" value={form.physicalQuantity} onChange={handleChange} placeholder="0" min="0" className={inputCls} required />
            </Field>
            <Field label="Difference">
              <input type="text" name="difference" value={form.difference} readOnly
                className={`${inputCls} bg-slate-50 ${parseInt(form.difference) > 0 ? 'text-emerald-600' : parseInt(form.difference) < 0 ? 'text-red-500' : ''}`} />
            </Field>
            ...
            <Field label="Reason for Discrepancy">
              <input type="text" name="reason" value={form.reason} onChange={handleChange} placeholder="e.g. Theft, Damage, Count error" className={inputCls} />
            </Field>
