...
import { ArrowLeft, Save, Package, Calendar, MapPin, FileText, User } from 'lucide-react';
...
export default function CustomerReturnForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      setLoading(false);
      alert('Customer Return entry created successfully!');
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[18px] font-extrabold text-slate-900 tracking-tight">Add Customer Return</h1>
            <p className="text-[11px] text-slate-500 font-semibold">Record new customer return entry</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleSubmit} disabled={loading}
            ...
            <Save size={14} />{loading ? 'Saving...' : 'Save Return'}
          </motion.button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Section title="Customer & Return Details" icon={User}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Field label="Customer Name" required>
              <input type="text" name="customer" value={form.customer} onChange={handleChange} placeholder="Enter customer name" className={inputCls} required />
            </Field>
            <Field label="Original Invoice No">
              <input type="text" name="originalInvoiceNo" value={form.originalInvoiceNo} onChange={handleChange} placeholder="e.g. INV-2026-155" className={inputCls} />
            </Field>
            <Field label="Return Reason" required>
              <select name="returnReason" value={form.returnReason} onChange={handleChange} className={inputCls} required>
            ...
            <Field label="Product Condition" required>
              <select name="condition" value={form.condition} onChange={handleChange} className={inputCls} required>
